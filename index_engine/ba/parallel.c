...
void parallelGraphicsEngine(){
	...
	for (int i = 0; i< num_of_cldevices;i++){
		ret = clEnqueueReadBuffer(cl_devices[i].command_queue, cl_devices[i].satelite_id_gpu, CL_FALSE, 0, cl_devices[i].pixel_arr_size * sizeof(char), cl_devices[i].pixel_ids, 0, NULL, &cl_devices[i].evnt);
		if (ret != CL_SUCCESS){
			fprintf(stderr, getErrorString(ret, 1));
			exit(0);
		}	
